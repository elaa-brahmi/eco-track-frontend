"use client"
import { FilterBarAndReports } from "@/components/admin/reports/FilterBarAndReports"
import { ReportTabs } from "@/components/admin/reports/ReportTabs"
import ResolveTaskPopup from "@/components/admin/tasks/resolveTaskPopup"
...
const ReportsPage = () => {
  ...
  const [resolvePopup, setResolvePopup] = useState<any>(null)
  const [loading, setLoading] = useState(true);
  


 useEffect(() => {
  const load = async () => {setReports(await fetchReports()) ; setLoading(false);};
  load();

  const client = connectToWS({
    "/topic/reports": (newReport: ReportData) => {
      console.log("Report received:", newReport);
          ...
          {loading ? (
            ...
            <>
              ...
              <FilterBarAndReports
                ...
                onSolveReport={setResolvePopup}
              />
            </>
          )}
          {/* placeholder for the resolve report popup */}
          {resolvePopup && (
          <ResolveTaskPopup
              reportId={resolvePopup.id}
          ...
          {selectedReport && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
              <div className="bg-white p-6 rounded-lg max-w-lg w-full shadow-2xl">
                ...
                <p className="text-sm text-gray-500">type: {selectedReport.type}</p>
